...
def laplacian(adj, typ = 'normailzed'):
    if typ == 'normalized':
        ...
        D = torch.diag((torch.sum(adj, dim=0)) + torch.ones(adj.shape[0]) ** (-1/2))
        L = torch.mm(torch.mm(D, adj + torch.eye(adj.shape[0])), D)
    elif typ == 'usual':
...
def smooth_label_loss(data, out, lap, alpha=0.9):
...
def train(model, data, optimizer, lap, scheduler=None, loss='cross_entropy', alpha=0.9): # train for one epoch
    model.train()
    optimizer.zero_grad()
    out = model(data.x, data.edge_index)
    if loss == 'smooth_label':
        loss = smooth_label_loss(data, out, lap, alpha=alpha)
